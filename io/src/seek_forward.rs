use alloc::boxed::Box;
use bare_io::{self, Read, Write, BufRead, Seek, SeekFrom, copy, sink, repeat};
...
impl<T: BufRead> BufRead for ReadWriteTell<T> {
    ...
    #[inline]
    fn read_until(&mut self, byte: u8, buf: &mut Vec<u8>) ->  bare_io::Result<usize> {
    ...
    #[inline]
    fn read_line(&mut self, buf: &mut String) ->  bare_io::Result<usize> {
...
/*impl<T: Tell + SeekForward + SeekBackward + SeekAbsolute + SeekEnd> Seek for T {
    fn seek(&mut self, pos: SeekFrom) ->  bare_io::Result<u64> {
        match pos {
