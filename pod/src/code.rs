...
use bare_io::{self, BufRead, BufReader, Cursor, Read, Write};
...
pub trait Encode {
    ...
    fn encode<W: Write>(&self, w: &mut W) ->  bare_io::Result<()> {
        self.encode_options(w, Default::default())
    }

    /// Encodes to the `Write` with the provided options
    fn encode_options<W: Write>(&self, w: &mut W, _options: Self::Options) ->  bare_io::Result<()> {
        self.encode(w)
    }

    /// Encodes to a new byte vector
    fn encode_vec(&self) ->  bare_io::Result<Vec<u8>> {
        let data = Vec::new();
    ...
    fn encode_vec_options(&self, options: Self::Options) ->  bare_io::Result<Vec<u8>> {
        let data = Vec::new();
        let mut cursor = Cursor::new(data);

        self.encode_options(&mut cursor, options)?;
...
pub trait Decode: Sized {
    ...
    fn decode<R: Read>(r: &mut R) ->  bare_io::Result<Self> {
        Self::decode_options(r, Default::default())
    }

    /// Decodes from the `Read` with the provided options
    fn decode_options<R: Read>(r: &mut R, _options: Self::Options) ->  bare_io::Result<Self> {
        Self::decode(r)
    }

    /// Decodes from a byte slice
    fn decode_slice(data: &[u8]) ->  bare_io::Result<Self> {
        let mut cursor = Cursor::new(data);
    ...
    fn decode_slice_options(data: &[u8], options: Self::Options) ->  bare_io::Result<Self> {
        let mut cursor = Cursor::new(data);
    ...
    fn validate(&self) ->  bare_io::Result<()> {
        Ok(())
    }
}
...
impl<T: Encode> Encode for Option<T> {
    ...
    fn encode_options<W: Write>(&self, w: &mut W, options: Self::Options) ->  bare_io::Result<()> {
        self.as_ref()
            .map(|v| v.encode_options(w, options))
            .unwrap_or(Ok(()))
    }
}
...
impl<T: Decode> Decode for Option<T> {
    ...
    fn decode_options<R: Read>(r: &mut R, options: Self::Options) ->  bare_io::Result<Self> {
...
impl Decode for CString {
    ...
    fn decode_options<R: Read>(r: &mut R, options: Self::Options) ->  bare_io::Result<Self> {
        ...
        unsafe {
            let mut err = Ok(());
            let string = CString::from_vec_unchecked(
                r.bytes()
                    .map(|c| match c {
                        Ok(c) => c,
                        Err(e) => {
    ...
    T::Options: Clone,
{
    ...
    fn encode_options<W: Write>(&self, w: &mut W, options: Self::Options) ->  bare_io::Result<()> {
        for ref v in self {
            v.encode_options(w, options.clone())?;
        }
